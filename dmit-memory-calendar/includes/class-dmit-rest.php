 ...
 * `_dmit_event_date` while ignoring the year — that's what lets a single
...
class DMIT_MC_REST {
	...
	/**
	 * All events whose _dmit_event_date falls in the given month (any
	...
	private function format_event( $post ) {
		...
		$terms = get_the_terms( $post->ID, DMIT_MC_TAXONOMY );
